"""
RacingAgent_v4 with continuous forward/backward & steering Left/Right
    * [optional] agent's initial position can be set randomly after each reset

change summary compared to version 3    
    * action commands are absolute
"""
...
class RacingAgentContinuous_v4:
    def __init__(
        self,
        num_rivals,
        ...
        self.action_space = ActionSpace(
            low_bound=np.array([self.low_limit, self.low_limit]),  # accel, steer
            high_bound=np.array([self.high_limit, self.high_limit])
        )        
    ...
    def step(self, action_):
        ...
        Args:
           action_ (2D np array): [Accel, Steer] absolute accel & steer           
        """
        # execute chosen action
        self.action[0] = action_[0]
        self.action[1] = 1#action_[1]
        
        print("accel: {:.3f}, steer: {:.3f}".format(self.action[0], self.action[1]))
        ...
        #time.sleep(0.05)  # 0.05: 20hz, 0.07: 14hz, 0.1: 10hz
