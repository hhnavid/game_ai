...
class ReplayBuffer(object):
    def __init__(self, limit, action_shape, observation_shape):
        ...
        self.prev_states = RingBuffer(limit, shape=observation_shape)  # [length x state_dim]
        self.actions = RingBuffer(limit, shape=action_shape)           # [length x action_dim]
        self.rewards = RingBuffer(limit, shape=(1,))                   # [length x 1]
        self.done = RingBuffer(limit, shape=(1,))                      # [length x 1]
        self.nex_states = RingBuffer(limit, shape=observation_shape)   # [length x state_dim]
